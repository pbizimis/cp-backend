...
@router.get("/images")
async def get_user_images(mongodb: AsyncIOMotorClient = Depends(get_mongodb),  user: Auth0User = Security(auth.get_user, scopes=["use:all"]), stylegan_user_class = Depends(StyleGanUser.get_class)) -> dict:
...
async def delete_user_images(deletion_options: DeletionOptions, mongodb: AsyncIOMotorClient = Depends(get_mongodb),  user: Auth0User = Security(auth.get_user, scopes=["use:all"]), stylegan_user_class = Depends(StyleGanUser.get_class)) -> dict:
