...
from app.schemas.stylegan_models import stylegan2ada_models, Model, StyleGanModel
...
class StyleGan2ADA(StyleGanModel):
    ...
    def style_mix(self, row_image: Union[int, bytes], col_image: Union[int, bytes]) -> dict:
        return style_mix_two_images_stylegan2ada(self.model, self.method_options, row_image, col_image)
...
generation_method = StyleGanMethod(
    ...
    method_options=(
        Dropdown(place=1, name="Model", options=stylegan2ada_models.models, default=0, description="Choose your StyleGan2ADA model. The lower the FID value, the better the image quality."),
