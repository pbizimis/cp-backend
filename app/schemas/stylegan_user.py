...
from app.db.mongodb import save_user_image_in_mongodb, delete_user_images_from_mongodb, get_user_images_from_mongodb, delete_all_user_images_from_mongodb
from app.schemas.mongodb import ImageData, DeletionOptions
from app.db.google_cloud_storage import upload_blob_to_gcs, download_blob_from_gcs, delete_blob_from_gcs

class StyleGanUser:

    def __init__(self, user: Auth0User, mongodb: AsyncIOMotorClient, stylegan_class: Type[StyleGanModel] = None, stylegan_method_options: dict = None) -> None:
        self.user = user
    ...
    async def delete_user_images(self, deletion_options: DeletionOptions) -> None:
        if (deletion_options.all_documents):
            ...
            await delete_user_images_from_mongodb(self.mongodb, self.user.id, deletion_options.id_list)
    ...
    async def save_user_images(self) -> dict:
        for image_name, image_blobs in self.result_images_dict.items():
            ...
            if not (image_blob and w_vector_blob):
                self.result_images_dict[image_name] = self.stylegan_method_options.row_image if image_name == "row_image" else self.stylegan_method_options.column_image
                continue
            image_id = upload_blob_to_gcs("stylegan-images", image_blob)
            upload_blob_to_gcs("stylegan-images-vectors", w_vector_blob, image_id)
            image_data = ImageData(url=image_id, auth0_id=self.user.id, method=self.stylegan_method_options)
